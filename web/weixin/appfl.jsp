...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $('#grid').datagrid({
                toolbar: toolbar,
                url: "/weixinmanager/getItemFlList.do",
                method: "GET",
                columns: columns,
                ...
                onLoadSuccess: function (data) {
                    ...
                    $('td[field="xx"]').mousemove(function () {
                        var index = $(this).parents("tr[datagrid-row-index]").find("td[field='id'] span").attr("title");
                        localStorage.setItem('index', index);
                        if ($(this).attr('rowspan')) {
                            return;
                        }
                        var self = this;
                        $(this).children('div').children('a').attr('href',"javascript:testAddSubPage('商品规格模板设置','/weixin/guige_list.jsp?id="+index+"')");
                    });
        ...
        function doAdd() {
            ...
            $("#rightIframe").attr('src', "/weixin/fl_addUI.jsp");
        ...
        function doEdit() {
            ...
            if (rows.length == 0) {
                ...
                $("#rightIframe").attr('src', "/weixinmanager/editItemFlUI.do" + "?id=" + rows[0].id);
                $("#searchWindow").window("open");
                $('#searchWindow').window('center');
        ...
        function doRemove() {
            ...
            $.messager.confirm("警告", "你确定要删除选中版块吗？", function (r) {
                if (r) {//确认删除
                    var data = {"id": ids};
                    $.get("/weixinmanager/deleteItemFl.do", data, function (message) {
...
<div class="easyui-window" id="searchWindow">
    <iframe id="rightIframe" width="100%" height="100%" src="/weixin/fl_addUI.jsp">
    </iframe>
</div>
...
<script>
    ...
    function testAddSubPage(title,url){
        ...
        if (jq("#wu-tabs").tabs('exists', title)){
            jq("#wu-tabs").tabs('close', title);
