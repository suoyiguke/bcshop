  ...
  <head>
	...
	<script type="text/javascript">
		$(function(){
			...
			//批量删除
			$("#delBtn").naviBatchDelete({url:"<%=basePath %>form/delete.do"});
			//单个删除
			$(".fam-cross").naviDelete({url:"<%=basePath %>form/delete.do"});
			//全选、反选
			$("#chkAll").naviCheckAll();
			...
			$(".fam-table-go").click(function(){
				...
				var url="<%=basePath%>form/detail.do?id="+$(this).attr("id");
	    		window.parent.addTab(tabName,url,"",true);
			});
 ...
 <body>
	...
	<div id="wraper">
		...
		<div class="toolbar" >
			...
			<button class="btn" w="620" h="280",t="50" url="<%=basePath%>form/addUI.do" title="表单新增" id="addUIBtn">新增(N)</button>
			<button class="btn" id="delBtn" chkListCls="chkList">删除(D)</button>
		</div>
		<form id="NaviForm" action="<%=basePath %>form/list.do" method="post">
		<div class="searchbar">
			<table>
				<tr>
					<th>表单名称</th>
					<td>
						<input type="text" name="objname"  value="${requestScope.form.objname }"/>
					</td>
					<th>数据库表名</th>
					<td>
						<input type="text" name="tabName"  value="${requestScope.form.tabName }"/>
					</td>
					<th>分类</th>
					<td>
		...
		<div>
			<table class="tabList">
				...
				<tr>
					...
					<td><a id="${s.id }" objname="${s.objname }-${s.tabName}" href="javascript:void(0);" class="go_detail">${s.objname }</a></td>
					<td>${s.tabName }</td>
					...
					<td>
						...
						<i class="fam-pencil" id="${s.id }" w="620" h="280",t="50" url="<%=basePath%>form/editUI.do?id=${s.id}" style="cursor:pointer" title="表单修改"></i>&nbsp;&nbsp;&nbsp;&nbsp;
