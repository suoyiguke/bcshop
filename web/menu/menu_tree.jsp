  ...
  <body style="background:#fff;">
  	<div id="left" class="resizable">
	  	...
	  	<div id="mm" class="easyui-menu" class="left_content">
	        <div onclick="appendMenu()" data-options="iconCls:'icon-add'">新增子菜单</div>
    ...
    <div id="right" class="resizable">
  	...
  	<script type="text/javascript">
		$(function(){
			$('#tree').tree({   
		      checkbox: false,   
		      url: '<%=basePath%>tree/jsonList.do?type=Menu&isAll=1',   
		      lines:true, 
		      animate:true,
		      onContextMenu: function(e,node){
	          	 ...
	          	 $('#tree').tree('options').url = "<%=basePath%>tree/jsonList.do?pid=" + node.id+"&type=Menu&isAll=1";
	      	  },onClick:function(node){
            	  var frame=$("iframe");
                  frame.attr("src","<%=basePath%>menu/editUI.do?id="+node.id);
		...
		function appendMenu(){
            ...
            var frame=$("iframe");
            frame.attr("src","<%=basePath%>menu/addUI.do?pid="+node.id);
        }
        function removeMenu(){
        	var t = $('#tree');
            var node = t.tree('getSelected');
            $.post("<%=basePath%>menu/delete.do",{id:node.id},function(data){
            	if(data.isOk==1){
            		reLoadTreeData();
            	}else{
            		alert(data.msg);
        ...
        function reLoadTreeData(){
        	$('#tree').tree("options").url="<%=basePath%>tree/jsonList.do?type=Menu&isAll=1";
        	$("#tree").tree("reload");
        }
	</script>
  </body>
